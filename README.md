...
- 🎯 **交互式 LLM 练习** — 11 套练习页面，含选择题/填空题/简答题/代码实操
...
│       python_basic_test_010.html  #   练习 010 — PyMySQL + Redis（38 题）
│       python_basic_test_011.html  #   练习 011 — NumPy + Pandas + Matplotlib（38 题）
│
└── 回收站/                         # 废弃文件暂存，不参与运行
```
...
**页面间导航：** 每个练习页顶部有导航栏，支持「◂ 上一练习 | 下拉菜单选择 | 下一练习 ▸」。当前共 11 套练习：001 → 002 → 003 → 004 → 005 → 006 → 007 → 008 → 009 → 010 → 011。
...
`quiz-engine.js` 是 11 个练习页面的公用 JS 引擎，包含：
