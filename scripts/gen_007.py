"""生成 python_basic_test_007.html（Day06）- 行号精确替换"""
import re
...
writes = [
    {'qid': 31, 'title': '提取手机号（模仿）', 'desc': '使用正则表达式从文本中提取所有 11 位手机号。',
     'reference': 'import re\\n\\ntext = "联系人A: 13800138000, 联系人B: 13900139000, 编号: A1001"\\nphones = re.findall(r"\\\\d{11}", text)\\nprint(phones)  # [13800138000, 13900139000]'},
    {'qid': 32, 'title': '创建学生表（模仿）', 'desc': '创建 student 表，包含 id（主键自增）、name（非空字符串）、age（整数）、score（decimal）。',
     'reference': 'CREATE TABLE student (\\n    id INT PRIMARY KEY AUTO_INCREMENT,\\n    name VARCHAR(50) NOT NULL,\\n    age INT,\\n    score DECIMAL(5, 2)\\n);'},
    {'qid': 33, 'title': '查询指定列（模仿）', 'desc': '从 product 表中查询商品名称 name 和价格 price。',
     'reference': 'SELECT name, price FROM product;'},
    {'qid': 34, 'title': '提取订单号（变体）', 'desc': '从文本中提取所有以 P 开头、后面跟 4 位数字的订单号。',
     'reference': 'import re\\n\\ntext = "订单号: P1001, P2030, X999, P8888"\\norders = re.findall(r"P\\\\d{4}", text)\\nprint(orders)  # [P1001, P2030, P8888]'},
    {'qid': 35, 'title': '完整校验手机号（变体）', 'desc': '判断列表中哪些是完整合法的 11 位手机号格式。',
    ...
    {'qid': 36, 'title': '条件查询加排序（变体）', 'desc': '查询 product 表中价格大于 100 的商品，并按价格从高到低排序。',
     'reference': 'SELECT * FROM product WHERE price > 100 ORDER BY price DESC;'},
    {'qid': 37, 'title': 'RAG 文档清洗（综合）', 'desc': '提取手机号、删除页码、内部标记、页脚，压缩重复空行。',
     'reference': 'import re\\n\\nraw = """第 1 页\\\\n【内部资料】课程答疑记录\\\\n\\\\n学生手机号：13800138000\\\\n问题：re.match 和 re.search 有什么区别？\\\\n\\\\n\\\\n页脚：仅供学习"""\\n\\nphones = re.findall(r"\\\\d{11}", raw)\\ncleaned = re.sub(r"第\\\\s*\\\\d+\\\\s*页", "", raw)\\ncleaned = re.sub(r"【.*?】", "", cleaned)\\ncleaned = re.sub(r"页脚：.*", "", cleaned)\\ncleaned = re.sub(r"\\\\n{2,}", "\\\\n", cleaned).strip()\\nprint(phones)\\nprint(cleaned)'},
    {'qid': 38, 'title': '电商商品数据查询（综合）', 'desc': '基于 category 和 product 表完成：1.商品名称+价格+分类名；2.价格>平均价的商品；3.每个分类商品数量；4.库存最低的前3个商品。',
...
def highlight(code):
    KW = ['SELECT','FROM','WHERE','AND','OR','ORDER','BY','GROUP','HAVING','LIMIT','INSERT','INTO','VALUES','UPDATE','SET','DELETE','CREATE','TABLE','DROP','ALTER','PRIMARY','KEY','NOT','NULL','AUTO_INCREMENT','INT','VARCHAR','DECIMAL','AS','INNER','JOIN','ON','IF','ELSE','FOR','WHILE','DEF','RETURN','IMPORT','IN','WITH','TRY','EXCEPT','True','False','None']
    BI = ['print','range','len','type','int','str','list','dict','tuple','set','input','open','re','enumerate','zip','map','filter']
    for w in KW: code = re.sub(r'\b(' + w + r')\b', r'<span class="kw">\1</span>', code, flags=re.IGNORECASE)
    for w in BI: code = re.sub(r'\b(' + w + r')\b', r'<span class="fn">\1</span>', code, flags=re.IGNORECASE)
...
def gw(q):
    ref = highlight(q['reference'].replace('\\n', '\n'))
...
# ========== 读取模板，按行处理 ==========
with open('E:/AI_itheima/VS_code/my_blog/static/python_basic_test_006.html', 'rb') as f:
    raw = f.read()

lines = raw.split(b'\n')
total_lines = len(lines)

# 找所有 section div 的行索引（0-based）
sec_div_indices = [i for i, l in enumerate(lines) if b'<div class="section">' in l]
print(f'Section div indices (0-based): {sec_div_indices}')

# 各 section 卡片区域（title 行之后第一个 card 之前，到 </div> 之前）
# section 结构: <div class="section"> \n <div class="section-title>...  \n <div class="card"... \n ... \n </div>
def find_section_card_range(sec_idx, all_sec_indices):
    """返回 (first_card_idx, end_div_idx)，即卡片区域的起始和结束行索引（不含 end_div）"""
    title_idx = None
    for i in range(sec_idx + 1, all_sec_indices[all_sec_indices.index(sec_idx) + 1] if all_sec_indices.index(sec_idx) + 1 < len(all_sec_indices) else len(lines)):
        if b'section-title' in lines[i]:
            title_idx = i
            break
    ...
    for i in range(title_idx + 1, len(lines)):
        if b'<div class="card"' in lines[i]:
            first_card_idx = i
            break
    if first_card_idx is None:
        return None, None
    end_div_idx = None
    for i in range(first_card_idx, len(lines)):
        if lines[i].strip() == b'</div>':
            end_div_idx = i
            break
    return first_card_idx, end_div_idx

# 计算每个 section 的卡片区域
section_ranges = []
for i, sec_idx in enumerate(sec_div_indices):
    next_sec_idx = sec_div_indices[i + 1] if i + 1 < len(sec_div_indices) else len(lines)
    first_card = None
    end_div = None
    for j in range(sec_idx + 1, next_sec_idx):
        if b'section-title' in lines[j]:
            # 找第一个 card
            for k in range(j + 1, next_sec_idx):
                if b'<div class="card"' in lines[k]:
                    first_card = k
                    break
            break
    if first_card:
        for k in range(first_card, next_sec_idx):
            if lines[k].strip() == b'</div>':
                end_div = k
                break
    section_ranges.append((first_card, end_div))
    print(f'Section {i} (div at line {sec_idx+1}): card from line {first_card+1 if first_card else None} to line {end_div+1 if end_div else None}')

# 准备新卡片内容（字节串）
new_cards = [
    choice_cards.encode('utf-8'),
    fill_cards.encode('utf-8'),
    short_cards.encode('utf-8'),
    write_cards.encode('utf-8'),
]

# 从后往前替换（避免索引偏移）
replacements = [(r[0], r[1], nc) for r, nc in zip(section_ranges, new_cards) if r[0] is not None]
replacements.sort(key=lambda x: -x[0])  # 从后往前

result_lines = lines[:]
for start_idx, end_idx, new_content in replacements:
    new_lines = new_content.split(b'\n')
    result_lines = result_lines[:start_idx] + new_lines + result_lines[end_idx:]

# 全局文本替换
result = b'\n'.join(result_lines)
result = result.replace(b'<title>Python 练习 006</title>', b'<title>Python 练习 007</title>')
result = result.replace(b'<h1>Python 练习 006</h1>', b'<h1>Python 练习 007</h1>')
result = result.replace(b'32 道题', f'{total} 道题'.encode())
result = result.replace(b"const STORAGE_KEY = 'py_day05_2_quiz_history';", b"const STORAGE_KEY = 'py_day06_quiz_history';")
result = result.replace(b"const SAVE_KEY = 'py_day05_2_saves';", b"const SAVE_KEY = 'py_day06_saves';")
result = result.replace(b'<span id="objDone">0</span>/20', f'<span id="objDone">0</span>/{obj}'.encode())
result = result.replace(b'<span id="subjDone">0</span>/18', f'<span id="subjDone">0</span>/{subj}'.encode())
result = result.replace(b'一、单选题（10 题）', f'一、单选题（{len(choices)} 题）'.encode())
result = result.replace(b'二、填空题（10 题）', f'二、填空题（{len(fills)} 题）'.encode())
result = result.replace(b'三、简答题（5 题）', f'三、简答题（{len(shorts)} 题）'.encode())
result = result.replace(b'四、代码实操（7 题）', f'四、代码实战（{len(writes)} 题）'.encode())
result = result.replace(
    b'<a href="/static/python_basic_test_005.html" class="quiz-arrow" title="上一练习">◂</a>',
    b'<a href="/static/python_basic_test_006.html" class="quiz-arrow" title="上一练习">◂</a>')
result = result.replace(
    b'<a href="/static/python_basic_test_006.html" class="active">练习 006</a>',
    b'<a href="/static/python_basic_test_006.html">练习 006</a>\n        <a href="/static/python_basic_test_007.html" class="active">练习 007</a>')
result = result.replace(
    b'<span class="quiz-arrow disabled" title="已是最后一个">▸</span>',
    b'<a href="/static/python_basic_test_007.html" class="quiz-arrow" title="下一练习">▸</a>')

output_path = 'E:/AI_itheima/VS_code/my_blog/static/python_basic_test_007.html'
with open(output_path, 'wb') as f:
    f.write(result)

# 验证
v = open(output_path, 'rb').read()
print(f'\n输出: {len(v)} bytes')
print(f'Python 练习 007 in file: {b"Python 练习 007" in v}')
print(f'py_day06_quiz_history: {b"py_day06_quiz_history" in v}')
print(f'data-qid="38": {b"data-qid=\\"38\\"" in v}')
print(f'Total data-qid count: {v.count(b"data-qid=")}')
print(f'Expected: 38')
