...
def highlight_python(code):
    KW_LIST = ['SELECT', 'FROM', 'WHERE', 'AND', 'OR', 'ORDER', 'BY', 'GROUP', 'HAVING', 'LIMIT',
               'INSERT', 'INTO', 'VALUES', 'UPDATE', 'SET', 'DELETE', 'CREATE', 'TABLE', 'DROP',
               'ALTER', 'PRIMARY', 'KEY', 'NOT', 'NULL', 'AUTO_INCREMENT', 'INT', 'VARCHAR',
               'DECIMAL', 'AS', 'INNER', 'JOIN', 'ON', 'LEFT', 'RIGHT', 'OUTER', 'IF', 'ELSE',
               'FOR', 'WHILE', 'DEF', 'RETURN', 'IMPORT', 'FROM', 'IN', 'WITH', 'AS', 'TRY',
               'EXCEPT', 'FINALLY', 'True', 'False', 'None']
    BI_LIST = ['print', 'range', 'len', 'type', 'int', 'str', 'list', 'dict', 'tuple', 'set',
               'input', 'open', 're', 'asyncio', 'enumerate', 'zip', 'map', 'filter']
...
def gen_fill_card(q):
    question = q['question'].replace('________', '<span class="blank-input" contenteditable="true"></span>')
...
total_q = len(choices) + len(fills) + len(shorts) + len(writes)
obj_total = len(choices) + len(fills)
subj_total = len(shorts) + len(writes)
...
content = content.replace('32 道题 · 选择题 + 填空题 + 简答题 + 代码实战 · 即时反馈 · 记录保存',
                          f'{total_q} 道题 · 选择题 + 填空题 + 简答题 + 代码实战 · 即时反馈 · 记录保存')

# 2. 修改 STORAGE_KEY 和 SAVE_KEY
content = content.replace(
    "const STORAGE_KEY = 'py_day05_2_quiz_history';",
    "const STORAGE_KEY = 'py_day06_quiz_history';"
).replace(
    "const SAVE_KEY = 'py_day05_2_saves';",
    "const SAVE_KEY = 'py_day06_saves';"
)

# 3. 修改导航中的上下页链接
content = content.replace(
    '<a href="/static/python_basic_test_005.html" class="quiz-arrow"',
    '<a href="/static/python_basic_test_006.html" class="quiz-arrow"'
).replace(
    '<a href="/static/python_basic_test_006.html" class="quiz-arrow" title="下一练习">▸</a>',
...
# 5. 修改客观题/主观题计数
content = content.replace('<span id="objDone">0</span>/20', f'<span id="objDone">0</span>/{obj_total}')
content = content.replace('<span id="subjDone">0</span>/18', f'<span id="subjDone">0</span>/{subj_total}')

# 6. 替换各 section 标题
content = content.replace(
    '<div class="section-title"><span class="icon"></span>一、单选题（10 题）</div>',
    f'<div class="section-title"><span class="icon"></span>一、单选题（{len(choices)} 题）</div>'
)
content = content.replace(
    '<div class="section-title"><span class="icon"></span>二、填空题（10 题）</div>',
    f'<div class="section-title"><span class="icon"></span>二、填空题（{len(fills)} 题）</div>'
)
content = content.replace(
    '<div class="section-title"><span class="icon"></span>三、简答题（5 题）</div>',
    f'<div class="section-title"><span class="icon"></span>三、简答题（{len(shorts)} 题）</div>'
)
content = content.replace(
    '<div class="section-title"><span class="icon"></span>四、代码实操（7 题）</div>',
    f'<div class="section-title"><span class="icon"></span>四、代码实战（{len(writes)} 题）</div>'
)

# 7. 替换各 section 中的卡片内容
# 使用精确的起止标记来替换
# 选择题：从 "    <div class=\"section\">\n  <div class=\"section-title\">" 到下一个 section 之前
# 但更简单的方法：用已知的卡片区域来替换

# 找到每个 section 的卡片区域并替换
def replace_section_cards(content, section_title, new_cards):
    """在指定的 section 标题后的内容中，用 new_cards 替换该 section 的卡片"""
    # 找到 section 标题的位置
    ...
    if section_start == -1:
        print(f'未找到 section 开始标签')
        return content
    # 找到 section 的结束位置：section_start 之后的第一个 </div>
    # 但要跳过 section-title 本身所在的标签
    # 从 section_title 位置往后找第一个 </div>
    search_from = idx
    while True:
        end_div = content.find('</div>', search_from)
        if end_div == -1:
            print(f'未找到 </div>')
            return content
        # 检查这个 </div> 和 section_start 之间是否还有未关闭的标签
        # 简单方法：找到 section_start 之后、end_div 之前的 <div...> 数量
        inner = content[section_start:end_div]
        open_count = inner.count('<div')
        close_count = inner.count('</div>')
        if open_count == close_count:
            # 这是一个平衡的 section
            section_end = end_div + 6  # len('</div>') == 6
            break
        search_from = end_div + 6
...
content = replace_section_cards(content,
    '<div class="section-title"><span class="icon"></span>一、单选题（10 题）</div>',
    choice_cards)
content = replace_section_cards(content,
    '<div class="section-title"><span class="icon"></span>二、填空题（10 题）</div>',
...
print(f'总计: {total_q} 道题 (客观{obj_total} + 主观{subj_total})')
print(f'选择题: {len(choices)} 填空题: {len(fills)} 简答题: {len(shorts)} 代码实战: {len(writes)}')